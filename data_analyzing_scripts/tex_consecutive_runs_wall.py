...
data_file = open("../data_backup/consecutive_runs/wall/consecutive_runs.txt", "r")
...
for line in data_file:
    if (i % 2 == 0):
        ...
        counter.append(j)
        opt_vals.append(round(float(line.split()[2]), 3))
        quality.append(round(float(line.split()[3]), 3))
        height.append(round(float(line.split()[1]), 6))
        j += 1
    i += 1
...
for k in range(0, len(counter), 1):
    latex_array_1.append( str(counter[k]) + " & " + str(height[k]) + " & " + str(int_vals[k]) + " & "
    + n_arr[k] + " & " + u_arr[k] + " & " + i_arr[k] + "\\\\ \n \\hline \n" )
    latex_array_2.append( str(counter[k]) + " & " + str(height[k]) + " & " + str(quality[k]) + " & " + str(opt_vals[k]) + " & " + str(n_real_err[k]) + " & " + str(u_real_err[k]) + " & " + str(i_real_err[k])
    + "\\\\ \n \\hline \n" )

tex_file_wall_1 = open("../data_backup/consecutive_runs/wall/tex_data_1.txt", "w")
tex_file_wall_2 = open("../data_backup/consecutive_runs/wall/tex_data_2.txt", "w")

for k in range(0, len(latex_array_1), 1):
    tex_file_wall_1.write(latex_array_1[k])
    tex_file_wall_2.write(latex_array_2[k])
