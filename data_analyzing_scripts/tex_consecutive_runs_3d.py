...
data_file = open("../data_backup/dim_3/consecutive_runs.txt", "r")
...
for line in data_file:
    if (i % 2 == 0):
        ...
        counter.append(j)
        opt_vals.append(round(float(line.split()[1]), 3))
        quality.append(round(float(line.split()[2]), 3))
        j += 1
    else:
        counter.append(j)
...
for k in range(0, len(counter), 1):
    latex_array_1.append( str(counter[k]) + " & " + n_arr[k] + " & " + u_arr[k] + " & " + i_arr[k] + "\\\\ \n \\hline \n" )
    latex_array_2.append( str(counter[k]) + " & " + str(quality[k]) + " & " + str(opt_vals[k]) + " & " + str(n_real_err[k]) + " & " + str(u_real_err[k]) + " & " + str(i_real_err[k])
    + "\\\\ \n \\hline \n" )

tex_file_wall_1 = open("../data_backup/dim_3/tex_data_1.txt", "w")
tex_file_wall_2 = open("../data_backup/dim_3/tex_data_2.txt", "w")
